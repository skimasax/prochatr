      ...
      <div class="container">
        @if($UserInterest = App\Interest::where('login_id', session('prochatr_login_id'))->get())
          ...
          <div class="row about-container">
            <a href="{{ route('main.dashboard') }}?redirect=true" class="col-md-12"><div class="col-md-12 text-center alert alert-danger" style="padding: 10px;margin-top: -50px;border-radius: 0px;">Return to Dashboard</div></a>
          </div>
          @endif
        @endif
        <div class="row about-container" id="list">
          ...
          <div class="col-md-4">
            ...
            <h6>Looking to find professionals that can meet your need</h6>
              <div class="row">
                @if($UserInterest = App\Interest::where('login_id', session('prochatr_login_id'))->get())
                  <div class="col-md-12" style="border-radius: 10px; background: #00000005; padding-bottom: 10px;">
                    {{-- {{ $UserInterest }} --}}
                    <div class="form-group row">
                      <label for="aboutme" class="col-xs-6 col-sm-6 col-md-6 col-form-label">About Me</label>
                      <div class="col-md-12">
                        <textarea id="aboutme" placeholder="Write in maximum of 200 words about yourself" rows="4" class="form-control">@if($UserInterest && count($UserInterest) > 0 ) {{ $UserInterest[0]->about }} @endif</textarea>
                      </div>
                    </div>                    
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-md-12">
                        <select id="experience" class="form-control">
                          @for($i=1;$i<=70; $i++)
                            <option value="{{ $i }}" @if($UserInterest && count($UserInterest) > 0) selected='' @endif>{{ $i }}</option>
                          @endfor
                        </select>
                      </div>
                    </div>
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-md-12">
                        <select id="offer" class="form-control">
                            <option value="Coaching/Mentorship" @if($UserInterest && count($UserInterest) > 0 && "Coaching/Mentorship"== $UserInterest[0]->offer) selected='' @endif>Coaching/Mentorship</option>
                            <option value="Internship" @if(count($UserInterest) > 0 && "Internship"== $UserInterest[0]->offer) selected='' @endif>Internship</option>
                            <option value="Collaboration" @if(count($UserInterest) > 0 && "Collaboration"== $UserInterest[0]->offer) selected='' @endif>Collaboration</option>
                            <option value="Training" @if(count($UserInterest) > 0 && "Training"== $UserInterest[0]->offer) selected='' @endif>Training</option>
                        </select>
                      </div>
                    </div>                     
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-md-12">
                        <select id="need" class="form-control">
                            <option value="Coaching/Mentorship" @if($UserInterest && count($UserInterest) > 0 && "Coaching/Mentorship"== $UserInterest[0]->need) selected='' @endif>Coaching/Mentorship</option>
                            <option value="Internship" @if(count($UserInterest) > 0 && "Internship"== $UserInterest[0]->need) selected='' @endif>Internship</option>
                            <option value="Collaboration" @if(count($UserInterest) > 0 && "Collaboration"== $UserInterest[0]->need) selected='' @endif>Collaboration</option>
                            <option value="Training" @if(count($UserInterest) > 0 && "Training"== $UserInterest[0]->need) selected='' @endif>Training</option>
                        </select>
                      </div>
                    </div>
                    <button type="button" class="form-control btn btn-primary" id="saveInterest">Save & continue</button>
                @endif
                  </div>
              </div>
          </div>
        ...
        <div class="col-md-12">
          <h5 class="text-left" style="margin-right: 10px; margin-left: 10px;">Suggestion(s) For You 
            ...
            <i class="fa fa-window-minimize pull-right minimizeList" title="Normalize Page" style="margin-top: -5px;"></i>
            <img src="" alt="">
            <br />
            <span style="font-size: 12px;">The following matches what you need..</span>
            {{-- <a href="{{ route('main.dashboard', '_connection=set') }}" title="" class="pull-right" style="cursor: pointer;"><button type="button" class="btn btn-sm btn-danger">Exit >></button></a> --}}
          </h5>
